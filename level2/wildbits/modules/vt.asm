********************************************************************
* VT - Virtual terminal factory descriptor
*
* IT.WND=$FF. Init skips InitTerm. SS.Open F$Links /vt1../vt8 and
* replaces V$DESC. Port $FFFF is not /term ($00FF) and not 0-8.
********************************************************************

                    nam       vt
                    ttl       Virtual terminal factory descriptor

                  IFP1
                    use       defsfile
                    use       wildbits_vtio.d
                  ENDC

tylg                set       Devic+Objct
atrv                set       ReEnt+rev
rev                 set       $00

                    mod       eom,name,tylg,atrv,mgrnam,drvnam

                    fcb       UPDAT.    mode byte
                    fcb       HW.Page   extended controller address
                    fdb       $FFFF     physical controller address (factory)
                    fcb       initsize-*-1 initilization table size
                    fcb       DT.SCF    device type:0=scf,1=rbf,2=pipe,3=scf
                    fcb       $00       case:0=up&lower,1=upper only
                    fcb       $01       backspace:0=bsp,1=bsp then sp & bsp
                    fcb       $00       delete:0=bsp over line,1=return
                    fcb       $01       echo:0=no echo
                    fcb       $01       auto line feed:0=off
                    fcb       $00       end of line null count
                    fcb       $00       pause:0=no end of page pause
                    fcb       60        lines per page
                    fcb       C$BSP     backspace character
                    fcb       C$DEL     delete line character
                    fcb       C$CR      end of record character
                    fcb       C$EOF     end of file character
                    fcb       C$RPRT    reprint line character
                    fcb       C$RPET    duplicate last line character
                    fcb       C$PAUS    pause character
                    fcb       C$INTR    interrupt character
                    fcb       C$QUIT    quit character
                    fcb       C$BSP     backspace echo character
                    fcb       C$BELL    line overflow character (bell)
                    fcb       $80       init value for dev ctl reg
                    fcb       $00       baud rate
                    fdb       name      copy of descriptor name address
                    fcb       $00       acia xon char
                    fcb       $00       acia xoff char
                    fcb       40        (szx) number of columns for display
                    fcb       24        (szy) number of rows for display
                    fcb       $FF       window number ($FF = factory)
initsize            equ       *

name                fcs       /vt/
mgrnam              fcs       /SCF/
drvnam              fcs       /VTIO/

                    emod
eom                 equ       *
                    end
